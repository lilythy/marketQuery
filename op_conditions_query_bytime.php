...
//$sql="SELECT bar_code,productName,measur_units,purch_price,final_price FROM product WHERE bar_code = '$barcode'";
//$sql="SELECT in_code,main_pro_name,purch_price,num,price,store_house,operatorMan FROM inventory_in WHERE oper_time >= '$start_time' and oper_time <= '$end_time'";
$sql="select a.bar_code as barcode,productName,SUM(b.num)*a.purch_price AS in_total, SUM(b.num)*a.final_price AS sale_total,SUM(b.num)*(a.final_price-a.purch_price) as profit from product a ,inventory_out b
                where a.bar_code = b.bar_code and b.oper_time = '$start_time'
                order by a.bar_code";
...
$result = mysql_query($sql,$conn);       
 //$row = mysql_fetch_array($result);
		...
		//$sql = "SELECT bar_code FROM product where bar_code=$barcode";
		//$sql="SELECT bar_code,productName,measur_units,purch_price,final_price FROM product WHERE bar_code = '".$barcode."'";
?>	
...
<div class='formMiddleContent formMiddleContent303 moduleContent'><div class='cubeNavigationArea cubeNavigationArea5' id='cubeNavigation303'>
   ...
   <table class="table">
      <caption><b  style="font-size:18px;">该天经营状况如下表</b></caption>
      <thead>
         <tr>
            <th>商品条码</th>
            <th>商品名称</th>
