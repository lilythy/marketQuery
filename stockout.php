...
<script>
            function v(){
        document.getElementById('price').value=document.getElementById('num').value+document.getElementById('final_price').value;
    }
...
<div class='formMiddleContent formMiddleContent303 moduleContent'><div class='cubeNavigationArea cubeNavigationArea5' id='cubeNavigation303'>
    ...
    <legend style="font-weight:bold;font-style:italic;font-size:18px;color:#CC3300">商品出库：</legend>
	<form width="100%"  role="form" method="post" action="inventory_in.php" cellspacing="0"> 
        <div class="item1">商品码：</div><div class="item2"><input type="text" id="bar_code" name="bar_code"/></div>
		<div style="padding-top:10px"><div class="item1" >商品名称：</div><div class="item2"><input id="product_name" name="product_name" type="text" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" placeholder="商品码只能为数字"/></div></div>
		<div style="padding-top:10px"><div class="item1" >售价：</div><div class="item2"><input id="final_price" name="final_price" type="text" onkeyup="if(isNaN(value))execCommand('undo');calc()" onafterpaste="if(isNaN(value))execCommand('undo')"/></div></div>
		<div style="padding-top:10px"><div class="item1" >数量：</div><div class="item2"><input id="num" name="num" type="text" onkeyup="this.value=this.value.replace(/\D/g,'');calc()" onafterpaste="this.value=this.value.replace(/\D/g,'')" placeholder="数量只能为正整数"/></div></div>
		<div style="padding-top:10px"><div class="item1" >总金额：</div><div class="item2"><input id="price" name="price" type="text" /></div></div>
		<div style="padding-top:10px"><div class="item1" >仓库：</div><div class="item2"><input id="store_house" name="store_house" type="text" /></div></div>
		<div style="padding-top:10px"><div class="item1" >门店：</div><div class="item2"><input id="shop" name="shop" type="text" /></div></div>
		<div style="padding-top:10px"><div class="item1" >备注：</div><div class="item2"><input id="remark" name="remark" type="text" /></div></div>
	    <div style="padding-top:15px;padding-left:60px;margin-bottom:30px;"><input type="submit" id="submit" name="submit" style="width:305px;height:40px;boder-radius:20px;float: left;margin-right:10px" value="确定"></div>
		<!--<div style="padding-top:20px;padding-left:60px"><input type="button" style="width:305px;height:40px;boder-radius:20px;float: left;margin-right:10px" value="继续"></div>-->
	</form> 
	</fieldset>
...
<?php
function information($bar_code){
    //包含数据库连接文件
    include('conn.php');
	mysql_query("set names 'utf8'");
            $sql = "select productName,final_price,houseName from inventory where bar_code='$bar_code'";    
